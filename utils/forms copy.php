...
class criar_ambiente_moodle extends moodleform {
  public function definition () {
    ...
    $ministrantesOptions = array(
        '' => 'Selecione:',
    );

    if (!isset($ministrantes['moodle']) or empty($ministrantes['moodle'])) {
        $this->_form->addElement(
            'static',
            'aviso_ministrantes',
        ...
        foreach ($moodle as $ministrante) {
            ...
            $ministrantesOptions[$ministrante->id] = $nomeprofessor . " [{$codatc}] (Moodle)";
        }
    }
    ...
    if (isset($ministrantes['apolo'])) {
        foreach ($ministrantes['apolo'] as $ministrante) {
            ...
            $nome = $ministrante['nompes'];
            $ministrantesOptions[$ministrante['codpes']] = $nome . " [{$codatc}] (Apolo)";
        }
    }
